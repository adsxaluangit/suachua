...
const BuildingManager: React.FC<BuildingManagerProps> = ({ buildings, onAdd, onUpdate, onDelete }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (editingBuilding) {
      onUpdate({ ...editingBuilding, ...formData });
    } else {
      onAdd(formData);
    }
    handleCancel();
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-black text-white uppercase tracking-tight">Quản lý Dãy nhà</h2>
          <p className="text-gray-500 text-sm font-medium">Cấu hình hạ tầng cơ sở vật chất nhà trường.</p>
        </div>
        <button
        ...
        >
          {isAdding ? 'Đóng form' : 'Thêm dãy nhà mới'}
        </button>
      </div>
      ...
      {isAdding && (
        <form onSubmit={handleSubmit} className="bg-[#121212] p-6 rounded-2xl shadow-2xl border border-gray-800 space-y-5 animate-fadeIn">
          <h3 className="text-lg font-black text-orange-500 uppercase">{editingBuilding ? 'Cập nhật dãy nhà' : 'Đăng ký dãy nhà'}</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-1.5">Tên dãy nhà*</label>
              <input
                required
                type="text"
                value={formData.name}
          ...
          <div className="flex gap-4">
            <button type="submit" className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-black uppercase tracking-wider hover:bg-blue-700 transition-all shadow-lg active:scale-95">
              {editingBuilding ? 'Lưu cập nhật' : 'Xác nhận lưu'}
            </button>
            {editingBuilding && (
        ...
        <table className="min-w-full divide-y divide-gray-800">
          <thead className="bg-[#1a1a1a]">
            <tr>
              ...
              <th className="px-6 py-4 text-left text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">Tên dãy nhà</th>
          ...
          <tbody className="divide-y divide-gray-800">
            {buildings.map((b) => (
              <tr key={b.id} className="hover:bg-gray-800/30 transition-colors">
                <td className="px-6 py-4 whitespace-nowrap text-sm font-black text-orange-500">{b.code}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-white font-bold">{b.name}</td>
                <td className="px-6 py-4 text-sm text-gray-400 font-medium">{b.description || '-'}</td>
